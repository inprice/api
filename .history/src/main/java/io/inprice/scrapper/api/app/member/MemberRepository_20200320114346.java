...
public class MemberRepository {
   ...
   public ServiceResponse findASuitableCompanyId(String email) {
      List<Member> members = dbUtils.findMultiple(String
            .format("select * from member where email = %s and status = %s order by role", email, UserStatus.JOINED),
            this::map);
   ...
   public ServiceResponse changeStatus(MemberChangeStatusDTO changeStatusDTO) {
      ...
      try {
         ...
         try (PreparedStatement pstUpdate = con.prepareStatement("update member set status = ? where id = ? and company_id = ?")) {
            int i = 0;
