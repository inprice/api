...
public class MemberService {
   ...
   public ServiceResponse changeRole(MemberChangeFieldDTO changeRoleDTO) {
      changeRoleDTO.setStatusChange(false);

      ServiceResponse res = validate(changeRoleDTO);
      if (res.isOK()) {

         res = memberRepository.findById(changeRoleDTO.getMemberId());
         if (res.isOK()) {
            ...
            if (! member.getRole().equals(UserRole.ADMIN)) {
               ...
               res = new ServiceResponse("You cannot change this user's role!");
            }
         }
      }
      return res;
   ...
   public ServiceResponse changeStatus(MemberChangeFieldDTO changeStatusDTO) {
      changeStatusDTO.setStatusChange(true);

      ServiceResponse res = validate(changeStatusDTO);
      if (res.isOK()) {

         res = memberRepository.findById(changeStatusDTO.getMemberId());
         if (res.isOK()) {
