...
public class MemberRepository {
   ...
   public ServiceResponse changeStatus(MemberChangeStatusDTO changeStatusDTO) {
      ...
      try (Connection con = dbUtils.getConnection();
         ...
         int i = 0;
         pst.setLong(++i, changeStatusDTO.getId());
         ...
         if (pst.executeUpdate() > 0) {
            ...
            try (PreparedStatement pst = con
                  .prepareStatement("insert into member_history set retry = retry + 1 where id = ? and retry < 3 and status = ? and company_id = ?")) {
         int i = 0;
   ...
   public ServiceResponse increaseSendingCount(long memberId) {
      ...
      try (Connection con = dbUtils.getConnection();
            PreparedStatement pst = con
                  .prepareStatement("update member set retry = retry + 1 where id = ? and retry < 3 and status = ? and company_id = ?")) {
         int i = 0;
         pst.setLong(++i, memberId);
         pst.setString(++i, MemberStatus.PENDING.name());
         pst.setLong(++i, UserInfo.getCompanyId());
