...
public class MemberController {
   ...
   public void addRoutes(Javalin app) {
      ...
      app.put(Consts.Paths.Member.CHANGE_ROLE, (ctx) -> {
         ctx.json(Commons.createResponse(ctx, service.changeRole(ctx.bodyAsClass(MemberChangeFieldDTO.class))));
      });

      // change status
      app.put(Consts.Paths.Member.CHANGE_STATUS, (ctx) -> {
         ctx.json(Commons.createResponse(ctx, service.changeStatus(ctx.bodyAsClass(MemberChangeFieldDTO.class))));
      });
