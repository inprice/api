...
public class MemberRepository {
   ...
   public ServiceResponse findByMemberId(long memberId) {
      Member model = dbUtils.findSingle("select * from member where id=" + memberId, this::map);
      if (model != null)
   ...
   public ServiceResponse findPreviousStatusById(long memberId) {
      List<MemberHistory> memberHistories = dbUtils.findMultiple(String
            .format("select * from member_history where member_id = %d and company_id = %d", memberId, UserInfo.getCompanyId()),
            this::mapHistory);

      if (memberHistories != null && memberHistories.size() > 1) {
         MemberHistory memberHistory = memberHistories.get(1);
         return new ServiceResponse(memberHistory);
      }
      return Responses.NotFound.HISTORY;
   }
   ...
   public ServiceResponse changeStatus(MemberChangeStatusDTO changeStatusDTO) {
      ...
      try {
         ...
         if (changeStatusDTO.isPaused()) {
            ServiceResponse found = findPreviousStatusById(changeStatusDTO.getId());
            if (found.isOK()) {
               MemberHistory prevHistory = found.getData();
               changeStatusDTO.setStatus(prevHistory.getStatus());
            } else {
               response = new ServiceResponse("Previous status of this member not found!");
         ...
         try (PreparedStatement pstUpdate = con
            ...
            int i = 0;
            pstUpdate.setLong(++i, changeStatusDTO.getMemberId());
            ...
            if (pstUpdate.executeUpdate() > 0) {
               try (PreparedStatement pstInsert = con
                  ...
                  int j = 0;
                  pstInsert.setLong(++j, changeStatusDTO.getMemberId());
