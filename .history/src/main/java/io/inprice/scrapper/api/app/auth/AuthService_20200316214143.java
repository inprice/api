...
import io.inprice.scrapper.api.dto.EmailDTOValidator;
...
public class AuthService {
   ...
   private boolean checkCompany(User user) {
      if (user.getCompanyId() == null) {
         ...
         if (company != null) {
            ServiceResponse updated = userRepository.setCompanyId(user.getId(), company.getId());
            if (updated.isOK()) {
               user.setCompanyId(company.getId());
               return true;
            }
         }
         burada davet edilip de aktif olduğu bir firma var mı diye bakılacak
      }
      return false;
   }
