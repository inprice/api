...
public class MemberService {
      ...
      if (res.isOK()) {
         ...
         if (res.isOK()) {
            ...
            if (company.getId().equals(UserInfo.getCompanyId())) {
               ...
               if (res.isOK()) {
                  sendMail(memberDTO, company.getName());
               }
            } else {
   ...
   private void sendMail(MemberDTO memberDTO, String companyName) {
      ...
      if (message != null) {
         emailSender.send(Props.getEmail_Sender(),
               "About your invitation for " + companyName + " at inprice.io", memberDTO.getEmail(), message);
