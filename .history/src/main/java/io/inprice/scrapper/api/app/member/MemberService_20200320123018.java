   ...
   public ServiceResponse changeStatus(MemberChangeStatusDTO changeStatus) {
      ...
      if (res.isOK()) {
         ...
         if (isSuitable) {
            ...
            if (res.isOK()) {
               log.info("{} status is changed from {} to {} ", changeStatus.getMemberId(), member.getStatus(),
                     changeStatus.getStatus());
            }
         } else {
            res = new ServiceResponse("You cannot change " + changeStatus.getMemberId() + " status from "
