...
public class LinkRepository {
   ...
   public ServiceResponse findById(Long id) {
      Link model = dbUtils.findSingle(String.format("select * from link " + "where id = %d " + "  and company_id = %d ",
   ...
   public ServiceResponse getList(Long productId) {
      List<Link> links = dbUtils.findMultiple(
            String.format("select * from link " + "where product_id = %d " + "  and company_id = %d " + "order by name",
                  productId, UserInfo.getCompanyId()),
            this::map);
   ...
   public ServiceResponse insert(LinkDTO linkDTO) {
      ...
      final String query = "insert into link " + "(url, product_id, company_id, workspace_id) " + "values "
            + "(?, ?, ?, ?) ";
