...
public class AuthService {
   ...
   private boolean checkCompany(User user) {
      if (user.getCompanyId() == null) {
         ...
         if (found.isOK()) {
            ...
            ServiceResponse updated = userRepository.setCompanyId(user.getId(), member.getCompanyId());
            if (updated.isOK()) {
         ...
         ServiceResponse found = memberRepository.findByEmailAndCompanyId(user.getEmail(), user.getCompanyId());
         if (found.isOK()) {
            Member member = found.getData();
            user.setRole(member.getRole());
            return true;
   ...
   public ServiceResponse refreshTokens(String refreshToken, String accessToken, String ip, String userAgent) {
      if (StringUtils.isNotBlank(refreshToken) && !tokenService.isTokenInvalidated(refreshToken)) {
         ...
         String bareRefreshToken = tokenService.extractRefreshToken(refreshToken);
