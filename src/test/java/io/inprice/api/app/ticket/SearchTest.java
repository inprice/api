...
public class SearchTest {
	...
	private static final String SERVICE_ENDPOINT = "/tickets/search";
	...
	public void Everything_must_be_ok_WITH_superuser_AND_bound_account() {
		...
		res = Unirest.post(SERVICE_ENDPOINT)
			.headers(Fixtures.SESSION_0_HEADERS)
			.cookie(cookies)
			.body(createBody(null, null, null))
			.asJson();
	...
	public void Everything_must_be_ok_WITH_admin() {
		JSONObject json = callTheService(TestAccounts.Starter_plan_and_one_extra_user.ADMIN(), createBody(new String[] { "OPENED" }, null, null));
	...
	public void Everything_must_be_ok_WITH_editor() {
		JSONObject json = callTheService(TestAccounts.Starter_plan_and_one_extra_user.EDITOR(), createBody(new String[] { "IN_PROGRESS" }, null, null));
	...
	public void Everything_must_be_ok_WITH_viewer() {
		...
		JSONObject json = callTheService(TestAccounts.Standard_plan_and_two_extra_users.VIEWER(), createBody(null, null, new String[] { "OTHER" }), 1); //attention!
	...
	private JSONObject createBody(String[] statuses, String[] priorities, String[] subjects) {
		JSONObject body = new JSONObject();
		if (statuses != null) body.put("statuses", statuses);
		if (priorities != null) body.put("priorities", priorities);
		if (subjects != null) body.put("subjects", subjects);
