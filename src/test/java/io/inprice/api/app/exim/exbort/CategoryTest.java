...
public class CategoryTest {
	...
	private static final String SERVICE_ENDPOINT = "/exim/category/download";
	...
	@Test
	public void Brand_not_found() {
		String csv = callTheService(TestWorkspaces.Cancelled_Starter_plan_30_links_6_alarms.ADMIN());
		assertEquals("Category not found!", csv);
	}
	...
	public void Everything_must_be_ok_FOR_any_kind_of_users() {
		...
		for (Entry<TestRoles, JSONObject> roleUser: roleUserMap.entrySet()) {
			String csv = callTheService(roleUser.getValue());
			assertNotEquals("Category not found!", csv);
		}
	}

	private String callTheService(JSONObject user) {
