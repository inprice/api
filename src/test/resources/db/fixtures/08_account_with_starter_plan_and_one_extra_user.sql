...
-- -----------------------
call sp_create_product_and_links('X', 2, 1, 1, 0, 'https://amazon.com/', 2, 'Account-C', @account_id);
call sp_create_product_and_links('Y', 4, 1, 0, 3, 'https://ebay.com/', 12, 'Account-C', @account_id);

-- -----------------------
-- 2 alarms
-- -----------------------
insert into alarm (topic, product_id, subject, subject_when, amount_lower_limit, amount_upper_limit, account_id) 
select 'PRODUCT', id, 'MINIMUM', 'OUT_OF_LIMITS', 15.17, 50.23, @account_id from product where account_id = @account_id limit 1;
...
values ('NORMAL', 'SUPPORT', 'PRODUCT', 'Is there a limit for adding new products?', @admin_id, @account_id);

insert into test.ticket_history (ticket_id, status, priority, type, subject, user_id, account_id) 
values (last_insert_id(), 'OPENED', 'NORMAL', 'SUPPORT', 'PRODUCT', @admin_id, @account_id);
