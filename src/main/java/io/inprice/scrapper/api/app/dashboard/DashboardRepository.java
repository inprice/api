...
public class DashboardRepository {
   ...
   private void addUserInfo(Connection con, JsonObject parent) throws SQLException {
      final String query = "select full_name, email, role " + "from user as u " + "where u.id=?";
   ...
   private void addWorkspaceInfo(Connection con, JsonObject parent) throws SQLException {
      final String query = "select w.last_collecting_time, w.last_collecting_status, p.name, w.name, w.due_date "
            + "from workspace as w " + "left join plan as p on w.plan_id = p.id " + "where w.id=?";
      try (PreparedStatement pst = con.prepareStatement(query)) {
   ...
   private void addMRUTenLinks(Connection con, JsonObject parent) throws SQLException {
      final String query = "select sku, name, seller, price, status, last_update, website_class_name " + "from link "
            + "where workspace_id=? " + "order by last_update desc " + "limit 10";
      try (PreparedStatement pst = con.prepareStatement(query)) {
         pst.setLong(1, UserInfo.getWorkspaceId());
