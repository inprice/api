...
public class AuthRepository {
  ...
  public boolean closeSession(List<ForCookie> sessions) {
    ...
    if (closed.size() > 0) {
      ...
      return db.executeQuery(String.format("delete from user_session where _hash in ('%s')", inClause),
  ...
  public ServiceResponse findOpenedSessions(List<String> excludedHashes) {
    ...
    String query = "select distinct os, browser, ip, accessed_at from user_session where user_id=? and _hash not in ('"
        + inClause + "')";
  ...
  public boolean closeByUserId(Long userId) {
    try (Connection con = db.getConnection()) {
      List<ForDatabase> sessions = db.findMultiple(con,
          String.format("select * from user_session where user_id=%d", userId), this::map);
      if (sessions != null && sessions.size() > 0) {
        for (ForDatabase ses : sessions) {
          RedisClient.removeSesion(ses.getHash());
        }
        return db.executeQuery(con, String.format("delete from user_session where user_id=%d", userId),
  ...
  public boolean closeByUserAndCompanyId(Long userId, Long companyId) {
    try (Connection con = db.getConnection()) {
      List<ForDatabase> sessions = db.findMultiple(con,
          String.format("select * from user_session where user_id=%d and company_id=%d", userId, companyId), this::map);
      if (sessions != null && sessions.size() > 0) {
        for (ForDatabase ses : sessions) {
          RedisClient.removeSesion(ses.getHash());
        }
        return db.executeQuery(con,
  ...
  public ServiceResponse saveSessions(List<ForRedis> redisSesList, List<ForDatabase> dbSesList) {
    ...
    if (isAdded) {
      ...
      boolean result = db.executeBatchQueries(queries, "Failed to add new sessions for: " + dbSesList.get(0),
          dbSesList.size());
  ...
  private ServiceResponse refreshAccessedAt(String hash) {
    boolean result = db.executeQuery("update user_session set accessed_at = now() where _hash = '" + hash + "'",
  ...
  private ForDatabase map(ResultSet rs) {
    try {
