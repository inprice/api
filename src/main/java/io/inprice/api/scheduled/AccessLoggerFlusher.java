      ...
      try (Handle handle = Database.getHandle()) {
        ...
        PreparedBatch batch = 
      		handle.prepareBatch(
    				...
    				"values (:log.userId, :log.userEmail, :log.userRole, :log.accountId, :log.accountName, :log.ip, :log.agent, :log.path, :log.pathExt, :log.method, :log.reqBody, :log.resBody, :log.status, :log.elapsed, :log.slow, :log.createdAt)"
					);
        int remaining = 0;
