  ...
  public void run() {
    ...
    try {
      ...
      try (Handle handle = Database.getHandle()) {
        ...
        PreparedBatch batch = 
      		handle.prepareBatch(
    				"insert into access_log (user_id, user_email, user_role, account_id, account_name, ip, agent, path, path_ext, method, req_body, res_body, status, elapsed, is_slow, created_at) "+
    				"values (:log.userId, :log.userEmail, :log.userRole, :log.accountId, :log.accountName, :log.ip, :log.agent, :log.path, :log.pathExt, :log.method, :log.reqBody, :log.resBody, :log.status, :log.elapsed, :log.isSlow, :log.createdAt)"
					);
        int remaining = 0;
